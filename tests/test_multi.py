...
def test_convention_names_constant() -> None:
    assert (
        frozenset({"proj", "spatial", "multiscales", "license", "uom", "stac", "cs"})
        == CONVENTION_NAMES
    )
...
def test_create_many_all() -> None:
    result = create_many(
        {
            ...
            "cs": {"crs": {"t": {"type": "temporal", "axes": {"time": {}}}}},
        }
    )
    assert len(as_sequence(result["zarr_conventions"])) == 7
    assert result["proj:code"] == "EPSG:4326"
    ...
    assert as_mapping(result["crs"])["t"] == {"type": "temporal", "axes": {"time": {}}}
