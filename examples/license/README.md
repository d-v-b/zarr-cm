...
[license](https://github.com/clbarnes/zarr-convention-license) convention, which
attaches a license specifier (SPDX identifier, URL, or text) to a Zarr node.
...
- Scaffold a migration: license has a single revision today, so the migrate step
  is an identity re-stamp, written the way a real cross-revision migration would
  be
