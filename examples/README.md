...
convention that zarr-cm implements. Every script walks the same three
workflows -- create new data, read data written under an unknown or older
revision, and migrate data between revisions -- and prints a trace ending in
`OK`.
...
- [proj](proj/README.md) -- coordinate reference systems, including a
  migration that changes the document's pinned `schema_url`
- [multiscales](multiscales/README.md) -- multiscale pyramid layout on a
  group (single revision)
- [license](license/README.md) -- license specifiers (single revision)
- [uom](uom/README.md) -- units of measurement as UCUM codes (single
  revision)
