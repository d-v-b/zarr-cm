"""PEP 695 `type` alias for `JSONValue` (Python >= 3.12 only).

This module is imported by `zarr_cm._core` *only* on Python 3.12+. It is kept
separate because the `type` statement is a syntax error on 3.11, so it cannot
live behind a runtime `if` in a module that 3.11 must import.
...
The 3.11 fallback in `_core` uses `TypeAliasType` (fixes pydantic at runtime);
the project's own type checking runs at `pythonVersion = 3.12` so it exercises
