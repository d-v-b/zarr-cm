...
implements. Every script walks the same three workflows — create new data,
read data written under an unknown or older revision, and migrate data
between revisions — and prints a trace ending in `OK`. The test suite runs
them all and asserts exactly that.
...
- [multiscales](multiscales.md) — multiscale pyramid layout on a group
  (single revision)
